...
REMOVABLE_LIST = ["tailwind.config.js", "package-lock.json",
...
parser.add_argument(
    "init", nargs='?', help="argument will run the default configuration of tailwind in the current working directory or remove config files")
# take in an input the location of the input.css file that will be used for tailwind
parser.add_argument(
    "-i", "--input", help="input argument specifies the input css file to be used during tailwind config")
# take in the directory for the output.css file to be generated by tailwind css
parser.add_argument("-o", "--output", default="./dist/output.css",
...
parser.add_argument(
    "-e", "--extend", help="this allows a color palette to be specified through a text file", required=False)
...
def generateOutputCss(input, output):
    ...
    if input is None:
        ...
        with open(default, "w") as inputFile:
            ...
            print(inputLines)
            if inputLines.find(INPUT_TAILWIND_CLASSES) == -1:
                inputFile.write(INPUT_TAILWIND_CLASSES)
            else:
                print(f"Tailwind directives already present in {default}")
        input = default
    else:
        if find(input, os.getcwd()) is None:
            print(
                f"The file {input} does not exist within current working directory")
        else:
            prependLine(INPUT_TAILWIND_CLASSES, input)

    subprocess.check_call(
        f"npx tailwindcss -i {input} -o {output} --watch", shell=True)
